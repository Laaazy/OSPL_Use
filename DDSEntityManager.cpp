...
void DDSEntityManager::createParticipant(const char *partitiontName)//创建参与者
{
	domain = DOMAIN_ID_DEFAULT;//使用默认域，默认域定义为 （DomainId_t）0x7fffffff
	...
	partition = partitiontName;//此处只是为partition赋值，设置分区在createPublisher与subScriber里
}
...
void DDSEntityManager::createTopic(char *topicName)//根据主题名字符串创建主题
{
...
void DDSEntityManager::createPublisher()
{
	...
	pub_qos.partition.name[0] = partition;//此处真正设置分区
...
void DDSEntityManager::createSubscriber()
{
	...
	sub_qos.partition.name[0] = partition;//此处真正设置分区
